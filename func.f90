module functions
    ...
    !A function that calculates the reynolds algorithm 
    ...
    !A function that computes the heat equation and returns the values of time and temperature in matrix form
    function heateqn_grad_a(count,rlength,tlength, delxx,deltt,temp,init,bdry,Hin,c,p,tac,rho,bulkk,M,Hstart,acc_con,reg,k)
        real,allocatable,dimension(:,:) :: heateqn_grad_a
        ...
        heateqn_grad_a(:,1) = tac(count,:) !Might need to fix up
        heateqn_grad_a(:,2:N+1) = temp
        
    end function heateqn_grad_a
